...
public class WikiVerticle extends AbstractVerticle {
	...
	private static final String SQL_CREATE_PAGE = "insert into Pages values (4, 'apple', 'apple is good for health')";
	...
	private Future<Void> prepareDatabase() {
		...
		dbClient.getConnection(ar -> {
			if (ar.failed()) {
				...
				connection.execute(SQL_CREATE_PAGE, create1 -> {
					connection.close();
					if (create1.failed()) {
						LOGGER.error("Database preparation error", create1.cause());
						promise.fail(create1.cause());
					} else {
						promise.complete();
					}
				});
